...
const RatingsPage = () => {
  ...
  const fetchRatings = async (cursor?: string) => {
    ...
    try {
      ...
      // Manual pagination cursor projection
      if (items.length === 15) {
        setNextCursor(items[14].id);
      } else {
        setNextCursor(null);
      }
    } catch (err) {
  ...
  const handleDelete = async (r: AdminRating) => {
    if (!window.confirm(t("admin.ratings.confirm_delete", "¿Seguro que querés eliminar (soft delete) esta calificación?"))) return;
    ...
    <div className="flex flex-col gap-6 h-full">
      ...
      <div className="bg-white p-6 rounded-lg shadow border border-blueGray-200">
        <div className="flex justify-between items-center mb-4">
            ...
            <button
                ...
                className="bg-green-600 text-white px-4 py-2 rounded shadow font-bold hover:bg-green-700 transition"
            >
              ...
              {ratings.map((r) => (
                <tr key={r.id} className="hover:bg-blueGray-50">
                  <td className="px-6 py-4 min-w-[250px]">
                    ...
                    <div className="font-medium text-blueGray-800 break-words line-clamp-2" title={r.observacion}>
                      {r.observacion || <span className="text-gray-400 italic">Sin observación</span>}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-xs">
                    <div className="text-blueGray-800">
                        <span className="font-bold">De: </span>{r.calificador_id.substring(0,8)}
                    </div>
                    <div className="text-blueGray-800">
                        <span className="font-bold">A: </span>{r.calificado_id.substring(0,8)}
                    </div>
                    {r.solicitud_id && (
                        <div className="text-blueGray-400 text-[10px] mt-1">Sol: {r.solicitud_id.substring(0,8)}</div>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {r.deleted_by && (
                        <div className="text-[10px] text-red-500">Del: {r.deleted_by.substring(0,6)}</div>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {!r.deleted_at ? (
                      <span className="px-2 py-1 bg-green-100 text-green-700 font-bold text-xs rounded-full inline-flex items-center gap-1">
                        <i className="fas fa-check"></i> Activa
                      </span>
                    ) : (
                        <span className="px-2 py-1 bg-red-100 text-red-700 font-bold text-xs rounded-full inline-flex items-center gap-1">
                          <i className="fas fa-ban"></i> Eliminada
                        </span>
                    )}
                  </td>
                    ...
                    <div className="flex justify-end gap-2">
                        ...
                        {!r.deleted_at && (
                            <button 
                                ...
                                title="Eliminar"
                            >
                                {actionId === r.id ? <i className="fas fa-spinner fa-spin"></i> : <i className="fas fa-trash"></i>}
                            </button>
                        )}
                    </div>
                  </td>
      ...
      <div className="flex justify-between items-center py-4">
        ...
        <span className="text-sm font-medium text-blueGray-500">
          Página actual {cursorQueue.length + 1}
        </span>
        <button
          onClick={handleNext}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-lg p-6 relative">
            <h2 className="text-xl font-bold mb-4">{editingRating ? "Editar Calificación" : "Nueva Calificación"}</h2>
            <form onSubmit={handleSaveForm} className="flex flex-col gap-4">
               ...
               <>
                <div>
                   ...
                   <input required type="text" className="border w-full p-2 rounded" value={formCalificador} onChange={e=>setFormCalificador(e.target.value)} />
                </div>
                <div>
                   <label className="block text-sm font-bold mb-1">ID Calificado</label>
                   <input required type="text" className="border w-full p-2 rounded" value={formCalificado} onChange={e=>setFormCalificado(e.target.value)} />
                </div>
                <div>
                   <label className="block text-sm font-bold mb-1">ID Solicitud (Opcional)</label>
                   <input type="text" className="border w-full p-2 rounded" value={formSolicitud} onChange={e=>setFormSolicitud(e.target.value)} />
                </div>
               </>
               )}
               <div>
                 <label className="block text-sm font-bold mb-1">Puntuación (1-5)</label>
                 <input type="number" min={1} max={5} required className="border w-full p-2 rounded" value={formScore} onChange={e=>setFormScore(Number(e.target.value))} />
               </div>
               <div>
                 <label className="block text-sm font-bold mb-1">Observación</label>
                 <textarea className="border w-full p-2 rounded" rows={3} value={formObs} onChange={e=>setFormObs(e.target.value)}></textarea>
               </div>
               <div className="flex justify-end gap-2 mt-4">
                 <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 bg-gray-300 rounded font-bold hover:bg-gray-400">Cancelar</button>
                 <button type="submit" disabled={saving} className="px-4 py-2 bg-green-600 text-white rounded font-bold hover:bg-green-700 disabled:opacity-50">
